...
##
import builtins
# Global switch to control logs
ENABLE_LOGGING = False

def toggle_logging(func):
    """control functionprintLog decorator"""
    def wrapper(*args, **kwargs):
        # save originalprintfunction
        original_print = builtins.print
        try:
            # Determine whether to disable based on the switch statusprint
            if not ENABLE_LOGGING:
                builtins.print = lambda *args, **kwargs: None
            # Execute original function
            result = func(*args, **kwargs)
        finally:
            # restore originalprintfunction
...
class MlpMoE(nn.Module):
    ...
    def __init__(
        ...
        self.fc1 = MoE(in_features, hidden_dim=hidden_features)
        self.act = act_layer()
        self.drop1 = nn.Dropout(drop_probs[0])
        self.norm = norm_layer(in_features) if norm_layer is not None else nn.Identity()
        self.fc2 = MoE(in_features, hidden_dim=hidden_features)
    ...
    def forward(self, embs, log_prob=False, **extra_contexts):
        ...
        if self.training:
            ...
            if label.numel() == spatial_logits_map.numel():
                ...
                label = label.long()
                if self.token_format == 'xy':
                    label = ((label[..., 1] * w) + label[..., 0]).long()
                elif self.token_format == 'hw':
                    label = ((label[..., 0] * w) + label[..., 1]).long()
                else:
                    raise ValueError("token_format should be 'xy' or 'hw'")
            ...
            if log_prob:
                ...
                result['spatial_logits_map'] = spatial_logits_map
            return result
        else:
...
class AutoRegressivePolicy(nn.Module):
    ...
    def compute_loss(self, tks: Tensor, chk_ids: Optional[Tensor]=None, valid_tk_mask: Tensor=None, 
                     ...
                     match_layer: str = "",
                     contexts: Dict[str, Tensor]={}, log_prob=False, log_hm=False):
        """
        ...
        for i in map(int, tk_ids.unique()):
            ...
            if log_prob:
                ll = loss_dict.pop('log_prob')
                ll = sum(ll)
                cond_log_probs[mask] = ll
                hm_logits = loss_dict.pop('spatial_logits_map', None)
            for k, v in loss_dict.items():
                losses[f'{token["name"]}.{k}'] += v
        loss_dict = {k: sum(v) / len(v) for k, v in losses.items()}
        if log_hm:
            return loss_dict, cond_log_probs, hm_logits
        if log_prob:
