...
def load_replay_file(file_path):
    """
    通用加载函数，支持.replay和.h5两种格式
    """
    try:
        if file_path.endswith('.h5'):
            # 加载H5格式文件
            return load_h5_replay(file_path)
        else:
            # 加载原始的pickle格式文件
...
def load_h5_replay(file_path):
    """
    从H5文件加载replay数据，返回与原始格式相同的字典
    """
    data = {}
        ...
        with h5py.File(file_path, 'r') as hf:
            ...
            for key in hf.keys():
                try:
                    ...
                    # logger.warning(f"加载数据集 {key} 时出错: {str(e)}")
                    # 尝试其他方式加载
                    try:
                        data[key] = hf[key].value
                    except:
                        # logger.error(f"无法加载数据集 {key}")
                        pass
            ...
            for key in hf.attrs:
                try:
                    ...
                    logger.warning(f"加载属性 {key} 时出错: {str(e)}")
        
        return data
    except Exception as e:
        logger.error(f"打开H5文件 {file_path} 时出错: {str(e)}")
        raise
    ...
    def _get_from_disk(self, start_index, end_index):
        ...
        if start_index % self._replay_capacity < end_index % self._replay_capacity:
            for i in range(start_index, end_index):
                ...
                file_path = join(task_replay_storage_folder, '%d.h5' % task_index)
                d = load_replay_file(file_path)
