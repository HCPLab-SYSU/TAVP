...
def retreive_full_observation(essential_obs, episode_path, i, load_mask=False, skip_rgb=False):
    ...
    if not skip_rgb:
        obs['front_rgb'] = np.array(Image.open(osp.join(episode_path, '%s_%s' % (CAMERA_FRONT, IMAGE_RGB), IMAGE_FORMAT % i)), dtype=np.float32)
        obs['left_shoulder_rgb'] = np.array(Image.open(osp.join(episode_path, '%s_%s' % (CAMERA_LS, IMAGE_RGB), IMAGE_FORMAT % i)), dtype=np.float32)
        obs['right_shoulder_rgb'] = np.array(Image.open(osp.join(episode_path, '%s_%s' % (CAMERA_RS, IMAGE_RGB), IMAGE_FORMAT % i)), dtype=np.float32)
        obs['wrist_rgb'] = np.array(Image.open(osp.join(episode_path, '%s_%s' % (CAMERA_WRIST, IMAGE_RGB), IMAGE_FORMAT % i)), dtype=np.float32)
...
def save(save_dir, obs,i):
    np.save(osp.join(save_dir, f'step_{i}_front_rgb.npy'), obs['front_rgb'])
    np.save(osp.join(save_dir, f'step_{i}_left_shoulder_rgb.npy'), obs['left_shoulder_rgb'])
    np.save(osp.join(save_dir, f'step_{i}_right_shoulder_rgb.npy'), obs['right_shoulder_rgb'])
    np.save(osp.join(save_dir, f'step_{i}_wrist_rgb.npy'), obs['wrist_rgb'])
    np.save(osp.join(save_dir, f'step_{i}_front_pc.npy'), obs['front_point_cloud'])
    np.save(osp.join(save_dir, f'step_{i}_left_shoulder_pc.npy'), obs['left_shoulder_point_cloud'])
    np.save(osp.join(save_dir, f'step_{i}_right_shoulder_pc.npy'), obs['right_shoulder_point_cloud'])
    np.save(osp.join(save_dir, f'step_{i}_wrist_pc.npy'), obs['wrist_point_cloud'])
    ...
    def __getitem__(self, _):
        ...
        for _ in range(self._batch_size):
            ...
            if u < self.k2k_sample_ratios.get(task, 0.8):
                ...
                obs_point = 0 if kp == 0 else kp-1
                task_progress = float(obs_point) / len(episode['keypoints'])
            else:
                # o2k
            ...
            obs_media_dict = {k: v for k, v in obs_media_dict.items() if not k.endswith('depth')}
            ...
            sample_dict = {
                ...
                "curr_state": essential_obs.gripper_pose,
            ...
            # reset
            episode = kp = obs_frame_id = None

        # batch['task_id'] = self.tasks.index(task)
        ...
        # Key: lang_goal_tokens Shape: torch.Size([16, 77])
        # Key: lang_goal_embs Shape: torch.Size([16, 77, 512])
        # Key: keypoint_idx Shape: torch.Size([16])
        # Key: kp_frame_idx Shape: torch.Size([16])
        # Key: frame_idx Shape: torch.Size([16])
        # Key: episode_idx Shape: torch.Size([16])
        # Key: variation_idx Shape: torch.Size([16])
        # Key: task_idx Shape: torch.Size([16])
        # Key: gripper_pose Shape: torch.Size([16, 7])
        # Key: ignore_collisions Shape: torch.Size([16])
        # Key: gripper_action Shape: torch.Size([16])
        # Key: low_dim_state Shape: torch.Size([16, 3])
        # Key: front_rgb Shape: torch.Size([16, 3, 128, 128])
        # Key: left_shoulder_rgb Shape: torch.Size([16, 3, 128, 128])
        # Key: right_shoulder_rgb Shape: torch.Size([16, 3, 128, 128])
        # Key: wrist_rgb Shape: torch.Size([16, 3, 128, 128])
        # Key: front_depth Shape: torch.Size([16, 128, 128])
        # Key: left_shoulder_depth Shape: torch.Size([16, 128, 128])
        # Key: right_shoulder_depth Shape: torch.Size([16, 128, 128])
        # Key: wrist_depth Shape: torch.Size([16, 128, 128])
        # Key: front_point_cloud Shape: torch.Size([16, 3, 128, 128])
        # Key: left_shoulder_point_cloud Shape: torch.Size([16, 3, 128, 128])
        # Key: right_shoulder_point_cloud Shape: torch.Size([16, 3, 128, 128])
        # Key: wrist_point_cloud Shape: torch.Size([16, 3, 128, 128])
        # Key: rgb_mem Shape: torch.Size([16, 3, 3, 128, 128])
        # Key: pcd_mem Shape: torch.Size([16, 3, 3, 128, 128])
        # Key: action_pose_mem Shape: torch.Size([16, 3, 7])
        # Key: action_open_mem Shape: torch.Size([16, 3])
