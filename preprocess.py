...
def transform_pc_adaptive_scale(pc, loc, sca):
    """
    Change location of the center of the pc and scale it with per-sample scaling
    :param pc:
        either:
        - tensor of shape(b, num_points, 3)
        - tensor of shape(b, 3)
        - list of pc each with size (num_points, 3)
    :param loc: (b, 3)
    :param sca: scalar, (3), (b,), or (b, 3)
    """
    assert len(loc.shape) == 2
    assert loc.shape[-1] == 3
    b = loc.shape[0]
    
    # Convert sca to tensor if needed
    if not isinstance(sca, torch.Tensor):
        sca = torch.tensor(sca, dtype=loc.dtype, device=loc.device)
    
    # Ensure sca has proper shape (b, 3)
    if sca.dim() == 0:  # scalar
        sca = sca.view(1, 1).expand(b, 3)
    elif sca.dim() == 1:
        if sca.shape[0] == 3:  # (3,) -> (1, 3) -> (b, 3)
            sca = sca.unsqueeze(0).expand(b, 3)
        elif sca.shape[0] == b:  # (b,) -> (b, 1) -> (b, 3)
            sca = sca.view(-1, 1).expand(b, 3)
        else:
            raise ValueError(f"Unsupported sca shape: {sca.shape}")
    elif sca.dim() == 2:
        if sca.shape == (b, 1):  # (b, 1) -> (b, 3)
            sca = sca.expand(b, 3)
        elif sca.shape != (b, 3):
            raise ValueError(f"Unsupported sca shape: {sca.shape}")
    else:
        raise ValueError(f"Unsupported sca dim: {sca.dim()}")
    
    # Clone to avoid modifying original data
    loc = loc.clone()
    sca = sca.clone()
    
    # Handle different pc types
    if isinstance(pc, list):
        assert all([(len(x.shape) == 2) and (x.shape[1] == 3) for x in pc])
        assert len(pc) == b, "List length must match batch size"
        new_pc = []
        for i in range(b):
            scaled = sca[i] * (pc[i] - loc[i])
            new_pc.append(scaled)
    elif isinstance(pc, torch.Tensor):
        assert len(pc.shape) in [2, 3]
        assert pc.shape[-1] == 3
        assert pc.shape[0] == b, "Batch size mismatch"
        
        if len(pc.shape) == 2:
            new_pc = sca * (pc - loc)
        else:  # (b, n, 3)
            # Expand loc and sca for point-wise operations
            loc_exp = loc.unsqueeze(1)  # (b, 1, 3)
            sca_exp = sca.unsqueeze(1)  # (b, 1, 3)
            new_pc = sca_exp * (pc - loc_exp)
    else:
        raise TypeError("Unsupported pc type")
    ...
    def rev_trans(x):
        ...
        if x.dim() == 2:
            ...
            # (b, n, 3) tensor
            return x / sca.unsqueeze(1) + loc.unsqueeze(1)
        else:
            raise ValueError("Input tensor must be 2D or 3D")
    
    return new_pc, rev_trans
...
def preprocess_mem_obs_in_batch(sample_dict):
    def _norm_rgb(x):
        return (x.float() / 255.0) * 2.0 - 1.0

    rgb = sample_dict["rgb_mem"]
    pcd = sample_dict["pcd_mem"].float()
...
def get_cube_R_T_Dynamic(cam_look_at, with_scale=False, device='cuda'):
    '''
    Dynamically generate camera rotation matrix R and translation vector T
    parameter:
        cam_look_at: Tensor [B, 2, N, 3]
            B: batch size
            2: The first dimension is eye position(eye)，The second dimension is the upper vector(up)
            N: Number of cameras
            3: 3D coordinate value
        with_scale: Whether to return the zoom ratio
    return:
        R: rotation matrix [B, N, 3, 3]
        T: translation vector [B, N, 3]
        (Optional) scale: Zoom ratio [B, N, 3]
    '''
    cam_look_at = torch.tensor(cam_look_at, device=device)
    eye = cam_look_at[:, 0, :, :]  # [B, N, 3]
    up = cam_look_at[:, 1, :, :]   # [B, N, 3]
    
    # Set the target point as the origin (0,0,0)
    at = torch.zeros_like(eye).to(device)     # [B, N, 3]
    
    # Calculate rotation matrix R
    R = look_at_rotation(eye, at=at, up=up, device=device)  # [B, N, 3, 3]
    eye_unsqueezed = eye.unsqueeze(-1)        # [B, N, 3, 1]
    T = -torch.matmul(R, eye_unsqueezed)      # [B, N, 3, 1]
    T = T.squeeze(-1)                         # [B, N, 3]


    # B, N, _ = eye.shape

    # Calculate translation vector T: T = -R * eye
    
    # # Camera position (use directlyeye）
    
    # Handle scaling
    if with_scale:
        # Create unit scaling [B, N, 3]
        B, N = eye.shape[:2]
        scale = torch.ones(B, N, 3, device=eye.device, dtype=eye.dtype)
        return R, T, scale
    return R, T

def get_cube_R_T_Dynamic_v1(cam_look_at, with_scale=False, device='cuda'):
    '''
    Dynamically generate camera rotation matrix R and translation vector T
    parameter:
        cam_look_at: Tensor [B, 2, N, 3]
    ...
    # Calculate rotation matrix R
    B, N, _ = eye.shape
    eye_flat = eye.reshape(B * N, 3)
    at_flat = at.reshape(B * N, 3)
    up_flat = up.reshape(B * N, 3)
    R = look_at_rotation(eye_flat, at=at_flat, up=up_flat, device=device)  # [B*N, 3, 3]
    T = -torch.bmm(R.transpose(1, 2), at_flat[:, :, None])[:, :, 0]
    R = R.reshape(B, N, 3, 3)
    T = T.reshape(B, N, 3)
...
class CubePointCloudRenderer:
    ...
    @torch.no_grad()
    def points3d_to_screen2d_Dynamic(self, pt, cam_look_at):
        """
        returns the location of a point on the image of the cameras
        :param pt: torch.Tensor of shape (bs, np, 3)
        :returns: the location of the pt on the image. this is different from the
        ...
        # Get camera
        R, T, scale = get_cube_R_T_Dynamic_v1(cam_look_at=cam_look_at, with_scale=True, device=self.device)
        assert len(R.shape) == len(T.shape) + 1 == 4
        # [B,N,3,3]
        # [B,N,3]
        all_para = zip(R, T, scale, pt)
        pt_scr_all = []
        pt_camera_all = []
        for R_i, T_i, scale_i, pt_i in all_para:
            assert len(R_i.shape) == len(T_i.shape) + 1 == 3
            ...
            pt_scr = cameras.transform_points_screen(
                pt_i.reshape(-1, 3), image_size=self.img_size
            )[..., 0:2]
            pt_scr_all.append(pt_scr)
        pt_scr = torch.cat(pt_scr_all, dim=1)


        # Reshape point cloud to match camera count [B, N, M, 3]

        # # Create cameras in batches (more efficient)
        # )

        # # Convert point clouds in batches
        # )[..., :2]                       # Take onlyXYcoordinate
        # # Reshape to final shape [B, N, M, 2]
        ...
        # (bs * np, num_cameras, 2)
        pt_scr = pt_scr - torch.tensor((1 / w, 1 / h)).to(pt_scr.device)
        pt_scr = pt_scr.reshape(bs, np, self.num_cameras, 2)
        return pt_scr
    ...
    @torch.no_grad()
    def get_most_likely_point_3d_Dynamic(self, heatmap, dyn_cam_info=None):
        ...
        # (nc, np, 2)
        cached_grid_screen_points = self.points3d_to_screen2d_Dynamic(self.anchor_points3d.unsqueeze(0), [dyn_cam_info])
        cached_grid_screen_points = cached_grid_screen_points.squeeze(0).transpose(0, 1)
        
        # (nc, np, bs)
        cached_screen_points_weight = None
        pts_hm, cached_grid_screen_points, cached_screen_points_weight = grid_sample_from_heatmap(
            cached_grid_screen_points, heatmap.transpose(0, 1)[0 : self.num_cameras], cached_screen_points_weight)
        ...
        # (bs)
        ind_max_pts = torch.argmax(pts_hm, -1)
        return self.anchor_points3d[ind_max_pts]

    @torch.no_grad()
    def points3d_to_screen2d_Dynamic_batch(self, pt, cam_look_at):
        """
        returns the location of a point on the image of the cameras
        :param pt: torch.Tensor of shape (bs, np, 3)
        :returns: the location of the pt on the image. this is different from the
        ...
        # Get camera
        R, T, scale = get_cube_R_T_Dynamic(cam_look_at=cam_look_at, with_scale=True, device=self.device)
        assert len(R.shape) == len(T.shape) + 1 == 4
        # [B,N,3,3]
        # [B,N,3]
        all_para = zip(R, T, scale)
        pt_scr_all = []
        for R_i, T_i, scale_i in all_para:
            assert len(R_i.shape) == len(T_i.shape) + 1 == 3
            cameras = FoVOrthographicCameras(
                device=self.device,
                R=R_i,
            ...
            pt_scr = cameras.transform_points_screen(
                pt.reshape(-1, 3), image_size=self.img_size
            )[..., 0:2]
            pt_scr_all.append(pt_scr)
        pt_scr = torch.cat(pt_scr_all, dim=1)
        # Reshape point cloud to match camera count [B, N, M, 3]
    ...
    @torch.no_grad()
    def get_most_likely_point_3d_Dynamic_batch(self, heatmap, dyn_cam_info=None):
        bs, nc, h, w = heatmap.shape  # [batch_size, num_cameras, h, w]
        assert nc == self.num_cameras
        assert self.img_size == (h, w)

        if self.anchor_points3d is None:
            pts = torch.linspace(-1 + (1 / h), 1 - (1 / h), h).to(heatmap.device)
            self.anchor_points3d = torch.cartesian_prod(pts, pts, pts)  # [npt, 3]

        # Generate duplicates for each sample3Danchor point [bs, npt, 3]
        
        # Project to2Dscreen coordinates [bs, nc, npt, 2]
        cached_grid_screen_points = self.points3d_to_screen2d_Dynamic_batch(self.anchor_points3d.unsqueeze(0), dyn_cam_info).transpose(2, 1)
        
        # Sample heatmap values [bs, nc, npt, 1]
        pts_hm = self.grid_sample_from_heatmap_batch(
            cached_grid_screen_points,
            heatmap.unsqueeze(2)) # [bs, nc, 1, h, w] -> [bs, nc, npt, 1]
        pts_hm = pts_hm.squeeze(-1)

        # Aggregate camera dimensions [bs, npt]
        pts_hm = torch.mean(pts_hm, dim=1)  # Average along camera dimensions
        
        # find the most likely3Dpoint index [bs]
        ind_max_pts = torch.argmax(pts_hm, dim=-1)
        # Collect corresponding3Dpoint [bs, 3]  
        points = torch.stack([self.anchor_points3d[i] for i in ind_max_pts])
        return points

    def grid_sample_from_heatmap_batch(self, screen_points, heatmap):
        """
        Batch version of heatmap sampling function
        :param screen_points: [batch_size, num_cameras, num_points, 2]
        :param heatmap: [batch_size, num_cameras, 1, height, width]
        :return: Sample value [batch_size, num_cameras, num_points, 1]
        """
        bs, nc, _, h, w = heatmap.shape
        npt = screen_points.shape[2]
        
        # Flatten batch and camera dimensions [bs*nc, npt, 2]
        screen_points_flat = screen_points.reshape(bs * nc, npt, 2)
        heatmap_flat = heatmap.reshape(bs * nc, 1, h, w)
        
        # Create initial weights [bs*nc, npt]
        points_weight = torch.ones(bs * nc, npt, device=heatmap.device)
        
        # Boundary checking (Set the outer boundary weight to0)
        x_coords = screen_points_flat[..., 0]
        y_coords = screen_points_flat[..., 1]
        points_weight[(x_coords < 0) | (x_coords > w - 1) | 
                    (y_coords < 0) | (y_coords > h - 1)] = 0

        # calculate4coordinates of neighboring points [bs*nc, npt, 4, 2]
        floor_x = torch.floor(x_coords).long()
        floor_y = torch.floor(y_coords).long()
        ceil_x = torch.ceil(x_coords).long()
        ceil_y = torch.ceil(y_coords).long()
        
        # combination4corner point coordinates
        grid_points = torch.stack([
            torch.stack([floor_x, floor_y], dim=-1),
            torch.stack([floor_x, ceil_y], dim=-1),
            torch.stack([ceil_x, floor_y], dim=-1),
            torch.stack([ceil_x, ceil_y], dim=-1)
        ], dim=2)  # [bs*nc, npt, 4, 2]

        # Calculate distance weight [bs*nc, npt, 4]
        dist = 1 / (torch.norm(
            grid_points - screen_points_flat.unsqueeze(2), 
            dim=-1) + 1e-7)
        weights = dist * points_weight.unsqueeze(2)
        weights /= weights.sum(dim=2, keepdim=True).clamp_min(1e-7)

        # Convert to1Dindex [bs*nc, 4*npt]
        grid_points = grid_points.view(bs * nc, 4 * npt, 2)
        indices = grid_points[..., 1].clamp(0, h-1) * w + grid_points[..., 0].clamp(0, w-1)
        
        # Prepare heatmap [bs*nc, h*w]
        heatmap_1d = heatmap_flat.view(bs * nc, h * w)
        
        # Collect heatmap values [bs*nc, 4*npt]
        sampled = torch.gather(heatmap_1d, 1, indices)
        
        # weighted aggregation [bs*nc, npt, 1]
        sampled = sampled.view(bs * nc, npt, 4)
        result = torch.sum(sampled * weights, dim=2, keepdim=True)
        
        # Restore original dimensions [bs, nc, npt, 1]
        return result.view(bs, nc, npt, 1)
    ...
    #     # (nc, np, 2)
    #     # print("cached_grid_screen_points.shape:")
    #     # print(cached_grid_screen_points.shape)
    ...
    #     points_weight: Optional[torch.Tensor] = None
    # ) -> Tuple[torch.Tensor]:
    #     """
    #     :param screen_points: shape [batch_size, num_cameras, num_points, 2]
    #     :param heatmap: shape [batch_size, num_cameras, 1, height, width]
    #     :param points_weight: optional weight [batch_size, num_cameras, num_points, 4]
    #     :return: 
    #         - Heat map weighted value [batch_size, num_cameras, num_points, 1]
    #         - grid screen points (cache) [batch_size, num_cameras, 4 * num_points]
    #         - point weight (cache) [batch_size, num_cameras, num_points, 4]
    #     """
    #     batch_size, num_cameras, num_points, _ = screen_points.shape
    #     _, _, _, height, width = heatmap.shape
        
    #     # Combine batch and camera dimensions to simplify calculations
        
    #         # Initialization point weight (Default is 1)
    #             batch_size * num_cameras, num_points, 
    #         )
            
    #         # Set the weight of points beyond the image boundary to 0
    #             (screen_points_flat[..., 0] >= 0) &
    #             (screen_points_flat[..., 1] >= 0) &
    #             (screen_points_flat[..., 0] < width) &
    #             (screen_points_flat[..., 1] < height))
    #         points_weight_flat[~valid_mask] = 0
            
    #         # Generate four neighboring grid points for each point
    #         # shape: [batch*num_cam, num_points, 4, 2]
            
    #         # Calculate the coordinates of four adjacent points (Upper left, upper right, lower left, lower right)
    #         grid_points[:, :, 0, 0] = torch.floor(grid_points[:, :, 0, 0])  # upper left x
    #         grid_points[:, :, 0, 1] = torch.floor(grid_points[:, :, 0, 1])  # upper left y
    #         grid_points[:, :, 1, 0] = torch.ceil(grid_points[:, :, 1, 0])   # upper right x
    #         grid_points[:, :, 1, 1] = torch.floor(grid_points[:, :, 1, 1])  # upper right y
    #         grid_points[:, :, 2, 0] = torch.floor(grid_points[:, :, 2, 0])  # lower left x
    #         grid_points[:, :, 2, 1] = torch.ceil(grid_points[:, :, 2, 1])   # lower left y
    #         grid_points[:, :, 3, 0] = torch.ceil(grid_points[:, :, 3, 0])    # lower right x
    #         grid_points[:, :, 3, 1] = torch.ceil(grid_points[:, :, 3, 1])    # lower right y
            
    #         # Make sure the coordinates are within the image range
    #         grid_points[..., 0] = grid_points[..., 0].clamp(0, width - 1)
    #         grid_points[..., 1] = grid_points[..., 1].clamp(0, height - 1)
            
    #         # Calculate distance weight (Inversely proportional to Euclidean distance)
    #         ) + 1e-10)
            
    #         # Update point weight (Combine effectiveness and distance)
    #         weight_sum[weight_sum == 0] = 1  # avoid division by zero
            
    #         # Calculate linear index (y * width + x)
    #             grid_points[..., 1] * width + grid_points[..., 0]
    #         )  # [batch*num_cam, num_points, 4]
    #             batch_size * num_cameras, 4 * num_points
    #         )  # [batch*num_cam, 4*num_points]
    #     else:
    #         # Use the provided cached value
    #             batch_size * num_cameras, num_points, 4
    #         )
    #             batch_size * num_cameras, 4 * num_points
    #         )
        
    #     # Sample values ​​from heatmap
    #         batch_size * num_cameras, height * width
    #     )  # [batch*num_cam, height*width]
        
    #     # Collect heatmap values
    #     # output: [batch*num_cam, 4*num_points]
    #         heatmap_flat, 
    #     )
        
    #     # reshape and weighted average
    #         batch_size * num_cameras, num_points, 4
    #     )
        
    #     # Restoring batch and camera dimensions
    #         batch_size, num_cameras, num_points, 1
    #     )
    #         batch_size, num_cameras, 4 * num_points
    #     )
    #         batch_size, num_cameras, num_points, 4
    ...
    # @torch.no_grad()
    #     # enter heatmap shape: [batch_size, num_cameras, h, w]
    #     batch_size, nc, h, w = heatmap.shape
    #     assert nc == self.num_cameras
    #     assert self.img_size == (h, w)

    #     # Generate shared 3D anchor grid

    #     # Projection for each sample 3D click to 2D screen coordinates
    #     # enter: anchor_points3d_batch shape [batch_size, npt, 3]
    #     # output: screen_points shape [batch_size, num_cameras, npt, 2]
    #     # Adjustment heatmap dimensions to match grid_sample_from_heatmap input requirements
        
    #     # Sample values ​​from heatmap
    #     # output pts_hm shape: [batch_size, nc, npt, 1]
    #     pts_hm, _, _ = self.grid_sample_from_heatmap(
    #         screen_points, 
    #         heatmap_with_channel, 
    #         None
    #     )
        
    #     # Calculate the average heatmap value for each spatial point
        
    #     # Find the highest score in each sample 3D point index
        
    #     # Return the corresponding 3D anchor point


    
    
    @torch.no_grad()
    def get_most_likely_point_3d_Dynamic_old(self, hm, dyn_cam_info=None):
        """
        Modified to support dynamic camera parameters
        given set of heat maps, return the 3d location of the point with the
            largest score, assumes the points are in a cube [-1, 1]. 
        :param hm: (bs, nc, h, w) heat map
        :param dyn_cam_info: (Optional) Dynamic camera parameters [B, 2, N, 3]
        :return: (bs, 3) most likely3Dpoint position
        """
        bs, nc, h, w = hm.shape
        assert nc == self.num_cameras
        assert self.img_size == (h, w)
        
        # generate3Danchor grid
        if self.anchor_points3d is None:
            pts = torch.linspace(-1 + (1 / h), 1 - (1 / h), h).to(hm.device)
            self.anchor_points3d = torch.cartesian_prod(pts, pts, pts)  # (h^3, 3)
        
        # Prepare anchor batch: [bs, np, 3]
        anchor_pts3d = self.anchor_points3d.unsqueeze(0).expand(bs, -1, -1)
        
        # Calculate anchor point3DResponse values ​​in heatmap
        if dyn_cam_info is None:
            # Fixed camera solution
            # (bs, np, nc, 2)
            anchor_screen_points = self.points3d_to_screen2d(anchor_pts3d)
        else:
            # Dynamic camera solution
            # (bs, np, nc, 2)
            anchor_screen_points = self.points3d_to_screen2d_Dynamic(anchor_pts3d, dyn_cam_info)
        
        # Sample heatmap values (bs, nc, np, 1)
        anchor_pts_hm = []
        for i in range(bs):
            # Sample each batch individually
            sampled = []
            for cam_idx in range(nc):
                # Get the heat map coordinates of the current camera and batch
                cam_points = anchor_screen_points[i, :, cam_idx, :]  # (np, 2)
                
                # normalized to [-1, 1] scope (PyTorch grid_sampleRequire)
                h, w = self.img_size
                cam_points = cam_points.clone()
                cam_points[:, 0] = cam_points[:, 0] / (w - 1) * 2 - 1  # x: [-1, 1]
                cam_points[:, 1] = cam_points[:, 1] / (h - 1) * 2 - 1  # y: [-1, 1]
                
                # Sample heatmap values
                grid = cam_points.view(1, 1, -1, 2)  # (1, 1, np, 2)
                sampled_vals = F.grid_sample(
                    hm[i:i+1, cam_idx:cam_idx+1].unsqueeze(0),
                    grid,
                    align_corners=False
                )  # (1, 1, 1, np)
                sampled.append(sampled_vals.squeeze())
            
            # Combine all camera results (nc, np) -> (1, nc, np)
            anchor_pts_hm.append(torch.stack(sampled, dim=0).unsqueeze(0))
        
        # (bs, nc, np)
        anchor_pts_hm = torch.cat(anchor_pts_hm, dim=0)
        
        # Calculate average response value (bs, np)
        avg_hm = torch.mean(anchor_pts_hm, dim=1)
        
        # Find the index of the maximum response value (bs)
        max_indices = torch.argmax(avg_hm, dim=1)
        
        # Get the corresponding3Dpoint (bs, 3)
        best_points = anchor_pts3d[torch.arange(bs), max_indices]
...
def apply_se3_augmentation(
    ...
    euler = math3d.sensitive_gimble_fix(euler)
    ...
    roll = np.deg2rad(rot_aug_range[:, 0:1] * rand_dist((bs, 1)))
    pitch = np.deg2rad(rot_aug_range[:, 1:2] * rand_dist((bs, 1)))
    yaw = np.deg2rad(rot_aug_range[:, 2:3] * rand_dist((bs, 1)))
...
def find_out_of_range_values(tensor, min_val=0, max_val=224):
    ...
    if mask.any():
        ...
        for i in range(len(values)):
            idx = tuple(idx[i].item() for idx in indices)
            val = values[i].item()
            print(f"index {idx}: value = {val} {'< 0' if val < 0 else '> 224'}")
        return True
    return False
