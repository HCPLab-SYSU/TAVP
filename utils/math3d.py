...
def abs_euler_rad_to_discrete_euler(abs_euler_rad, resolution):
    euler_deg = np.degrees(abs_euler_rad)
    euler = euler_deg + 180
    assert np.min(euler) >= 0 and np.max(euler) <= 360
    disc = np.around((euler / resolution)).astype(int)
    disc[disc == int(360 / resolution)] = 0
...
def euler_rad_to_discrete_euler(euler_rad, resolution):
    # Convert radians to degrees and adjust the range from[-60, 60]arrive[0, 120]
    euler_deg = np.degrees(euler_rad)  # Radian rotation angle
    euler_shifted = euler_deg + 60  # from[-60,60]mapped to[0,120]
    
    # Make sure all values ​​are in[0, 120]within range
    assert np.min(euler_shifted) >= 0 and np.max(euler_shifted) <= 120
    
    # discretization
    disc = np.around((euler_shifted / resolution)).astype(int)
    
    # Processing Bounds: Will be equal to120/resolutionThe value is set to0（cycle boundary)
    # disc[disc == max_val] = 0
    
    return disc

def rotvec_rad_to_discrete_euler(rotvec_rad, resolution):
    # Convert radians to degrees and adjust the range from[-60, 60]arrive[0, 120]
    euler_deg = Rotation.from_rotvec(rotvec_rad).as_euler('xyz', degrees=True)
    euler_shifted = euler_deg + 60  # from[-60,60]mapped to[0,120]
